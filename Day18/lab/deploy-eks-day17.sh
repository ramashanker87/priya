...
export CLUSTER_NAME=priya-eks-cluster
...
kubectl rollout status deployment/priya-ecr-app -n day17-priya

echo "Service:"
kubectl get svc priya-ecr-service -n day17-priya

echo "When EXTERNAL-IP/hostname appears, test with:"
echo "curl http://$(kubectl get svc priya-ecr-service -n day17-priya -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')"
